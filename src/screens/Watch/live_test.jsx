...
function LiveTestWithRedux(props) {
  ...
  const m3u8Urltest = 'https://cdnapisec.kaltura.com/p/0/sp/0/playManifest/entryId/1_5l4tjbxn/format/applehttp/protocol/https/flavorParamId/manifest.m3u8';
  let realpath = videosrc;
  const videosrclower = videosrc?.toLowerCase();

  if(videosrclower === 'uisbotlive') {
    realpath = m3u8Url;
  } else if(videosrclower === 'test') {
    realpath = m3u8Urltest;
  }
  const media = {
    isLive: true,
    videos: [{
